...
# CONFIGURATION
# ================================
BINANCE_KEY = "n9MHjnEr1S6RnZwXNZIhYfGETCY4fVZIgflbGPfxXwIqNf7MqohtYNSmpeK0FfbR"
BINANCE_SECRET = "ZqRoYVTQ8dZndnBQ7EIeEGaxjPypmnFEqjIS9DiNjs2GkgdDD4wMcKFYgilITCUx"
GEMINI_KEY = "AIzaSyBKhQ5Rg6vQJyt6j2WHcOpXezACz2heDoc"
...
def get_balance():
    ...
    sig = sign(params)
    account = request_binance("GET", "/fapi/v2/balance", f"{params}&signature={sig}")
    # Note: Use /fapi/v2/balance for futures
...
def set_leverage(symbol, leverage=15):
    ...
    # Check if leverage set successfully
    if "code" in result and result["code"] < 0:
        # Reduce leverage if failed
        new_leverage = max(1, leverage // 2)
        if new_leverage != leverage:
            return set_leverage(symbol, new_leverage)
    return result
...
def round_to_precision(value, step_size):
    """Round value to nearest multiple of step_size."""
...
def place_order(symbol, side, order_type, qty, price=None):
    ...
    ts = get_timestamp()
    params = f"symbol={symbol}&side={side}&type={order_type}&quantity={qty}&timestamp={ts}"
...
"""
    ...
    try:
        ...
        # Adjust investment based on confidence
        confidence = parsed.get("confidence", "LOW")
        if confidence == "HIGH":
            investment = max(MIN_INVESTMENT, min(MAX_INVESTMENT, balance))
        elif confidence == "MEDIUM":
            investment = max(MIN_INVESTMENT, min(MAX_INVESTMENT, balance * 0.5))
        else:
            investment = max(MIN_INVESTMENT, min(MAX_INVESTMENT, balance * 0.1))
        parsed["investment_amount"] = investment

        # Leverage constraints
        leverage = int(parsed.get("leverage", 10))
        parsed["leverage"] = max(5, min(15, leverage))

        # TP percentage constraints
        tp_pct = float(parsed.get("tp_percentage", 1.0))
        parsed["tp_percentage"] = max(TP_MIN, min(TP_MAX, tp_pct))
...
def execute_scalp_trade(symbol, analysis, current_price):
    ...
    try:
        ...
        # Place TP/SL
        place_tp_sl_orders(symbol, signal, qty, current_price, tp_price, sl_price)
        ...
        active_trades[symbol] = {
            ...
            'orders': [],  # Save order IDs if needed
    ...
    for coin in coins[:5]:
        ...
        if analysis['signal'] == 'HOLD':
            ...
            if price_change > 2.0 and volume > 2000000:
                analysis['signal'] = 'BUY' if float(coin.get('priceChangePercent', 0)) > 0 else 'SELL'
                analysis['confidence'] = 'HIGH'
                analysis['investment_amount'] = min(MAX_INVESTMENT, max(MIN_INVESTMENT, balance * 0.5))
                log_info(f"🔄 Fallback trade for {symbol} due to high volatility.")
            else:
                continue
...
def run_bot():
    ...
    while running:
        try:
            ...
            if active_trades:
                monitor_trades()
                log_info(f"📊 Active trades: {len(active_trades)}")

            # Find new opportunity if no active trades
            if not active_trades:
