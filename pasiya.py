...
MIN_LOT = 0.01
RISK_PERCENT = 0.0001         # 0.0001% requested
...
def place_market_order(symbol, side, lot, sl_pips=None, tp_pips=None):
    ...
    if hasattr(res, "retcode") and res.retcode == mt5.TRADE_RETCODE_DONE:
        return True, f"✅ Order executed: {side.upper()} {symbol} lot={lot} price={price:.5f}"
    else:
        ...
        for sym in SYMBOLS:
            ...
            try:
                ...
                if sig:
                    ...
                    bot.send_message(chat_id=admin_id, text=f"Placing {sig.upper()} {sym} | Lot: {lot} | SL:{sl} | TP:{tp}")
                    ...
                    bot.send_message(chat_id=admin_id, text=f"{sym}: No trade signal now.")
...
async def cmd_safe(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with lock:
        ...
        if not ok:
            ...
            return
        start_balance = float(mt5.account_info().balance)
        running, mode = True, "safe"
